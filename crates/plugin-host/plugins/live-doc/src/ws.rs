...
/// Whether the receive loop carries on after handling one message.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Flow {
    /// Wait for the next message.
    Continue,
    /// The socket is finished; leave the loop.
    Stop,
}

/// Apply one binary frame to the room.
///
/// Split out of [`run_recv_loop`] rather than inlined: a frame that is too
/// large, one the room rejects, and one whose reply cannot be handed back are
/// three separate early exits, and expressing them as arms of a `match` inside
/// a `match` inside a loop nested the real work four levels deep.
async fn apply_client_frame(ctx: &RecvLoop, bytes: &[u8]) -> Flow {
    ...
    // Forward the per-client sync reply (e.g. the sync-step-2 carrying the
    // full document state) back to this socket.  If the send loop is gone the
    // session is closing.
    ...
    while let Some(Ok(msg)) = ctx.receiver.next().await {
        match msg {
            WsMessage::Binary(bytes) => {
                if apply_client_frame(&ctx, &bytes).await == Flow::Stop {
                    break;
                }
            }
...
mod tests {
    ...
    /// Read frames until the document reads `want`, or the socket ends.
    ///
    /// A free function rather than an inline `async` block: inside one, the
    /// loop and its test sit four levels deep before any work happens, which
    /// is over this workspace's nesting limit.
    async fn reads_until<S>(ws: &mut S, doc: &Doc, text: &yrs::TextRef, want: &str) -> bool
    where
        S: futures_util::Stream<Item = Result<TMessage, tokio_tungstenite::tungstenite::Error>>
            + Unpin,
    {
        while let Some(Ok(msg)) = ws.next().await {
            let TMessage::Binary(bytes) = msg else {
                continue;
            };
            apply_sync_frames(doc, &bytes);
            if text.get_string(&doc.transact()) == want {
                return true;
            }
        }
        false
    ...
    async fn reconnecting_client_receives_stored_content() {
        ...
        let got_content = tokio::time::timeout(
            Duration::from_secs(2),
            reads_until(&mut ws2, &reader_doc, &reader_text, "hello world"),
        )
        .await
        .unwrap_or(false);
