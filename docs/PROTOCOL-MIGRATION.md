...
* **M2b, complete the canon for the shipped features.** The D4 finding: the
  ...
  **First, a correction to how this list was drawn.** It originally measured
  ...
  | ~~screenshare~~ | **done, minus the SFU** | The canon was settled (SFU, ICE-lite, no trickling — each constraint bought with a debugging session); the *service* enforced none of it. Three defects, all of the same shape as the rest of this audit. **Anyone could stop anyone's share** by naming its id: no ownership check at all, no bug required, only a message. It now takes being the presenter or holding `MuteDeafen` in that channel — murmur has no "stop that stream" permission and that is the closest true analogue. **Starting a share checked nothing**, so a client could announce one in a channel it could not enter; it now takes `Speak`, which is what broadcasting into a channel takes. And shares were announced by **server-wide broadcast**, so a share titled "Quarterly numbers — draft" reached everyone connected, including people who cannot see the room: the announcement is now the channel's roster. Two more: `closed` was a no-op, so a presenter closing their laptop left the share live forever with its viewers watching a stream that had stopped (a share now remembers the *connection* it started on, because by the time `closed` runs the roster may have forgotten the session); and `Health` fell into a catch-all arm, so a client reporting that it was dropping half its frames told nobody. The SDP answer stays **empty**, and the correction is worth recording: an earlier version of this row said there is no SFU. There is — `vendor/server/3rdparty/webrtc-sfu`, str0m, ICE-lite, with `broadcaster_offer`/`viewer_offer` and answers out of `poll_event`, written for the C++ fork and reached through a C FFI. What is missing is the wiring, and it is a build-topology decision rather than a coding one: the crate sits in another repository's `3rdparty` directory, so a path dependency would tie Starling's build to the C++ fork's layout. Vendoring it here or running it as a service of its own (it already owns its runtime and sockets, which is most of what makes one) are the two shapes that do not |
